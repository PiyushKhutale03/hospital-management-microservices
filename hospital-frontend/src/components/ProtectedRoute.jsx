...
import { ShieldAlert, Lock, ArrowRight, Sparkles, LogIn, ShieldCheck } from "lucide-react";

export default function ProtectedRoute({ children, allowedRoles = [] }) {
  const { user, isAuthenticated, token } = useAuth();
  const location = useLocation();

  // 1. Check if user has an active authenticated JWT session
        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-xl border border-slate-200 text-center space-y-6 animate-fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              This portal is protected with JWT verification. Please sign in with an authorized user account to proceed.
            </p>
          </div>
        ...
        <div className="max-w-lg w-full bg-white rounded-3xl p-8 sm:p-10 shadow-2xl border border-amber-200/80 text-center space-y-6 animate-fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              ...
              This resource requires elevated <strong className="text-amber-800 uppercase">{allowedRoles.join(", ")}</strong> credentials.
            </p>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 text-left text-xs space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-slate-500 text-[11px]">
              ...
              <li>Patients and guests are strictly barred from accessing other portals or medical staff data.</li>
            </ul>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <Link
              to="/login"
              state={{ from: location }}
              className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs shadow-md shadow-brand-500/20 transition-all flex items-center justify-center gap-1.5"
            >
              <LogIn className="w-4 h-4" />
