    ...
    <nav className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Right Action: Demo Persona Switcher & User Profile */}
          <div className="hidden lg:flex items-center gap-3">
            ...
            <div className="relative">
              ...
              {demoDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50 animate-fade-in">
                  ...
                  <button
                    onClick={() => { switchDemoRole('PATIENT'); setDemoDropdownOpen(false); }}
                    className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-slate-50 text-slate-700"
                  >
                  ...
                  <button
                    onClick={() => { switchDemoRole('DOCTOR'); setDemoDropdownOpen(false); }}
                    className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-slate-50 text-slate-700"
                  >
                  ...
                  <button
                    onClick={() => { switchDemoRole('ADMIN'); setDemoDropdownOpen(false); }}
                    className="w-full text-left px-3 py-2 text-xs flex items-center gap-2 hover:bg-slate-50 text-slate-700"
                  >
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-left leading-tight hidden xl:block">
                    ...
                    <div className="text-[10px] font-medium text-slate-400">{user?.role}</div>
                  </div>
                </div>
                <button
                  onClick={logout}
              ...
              <Link
                to="/login"
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold bg-slate-900 text-white hover:bg-slate-800 transition-colors"
              >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-200 bg-white px-4 pt-2 pb-6 space-y-2 animate-slide-up">
          ...
          <div className="pt-4 border-t border-slate-200">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Switch Active Persona</div>
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={() => { switchDemoRole('PATIENT'); setMobileMenuOpen(false); }}
                className={`py-2 px-2 text-xs font-semibold rounded-lg border text-center ${role === 'PATIENT' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-slate-200'}`}
              >
                Patient
              </button>
              <button
                onClick={() => { switchDemoRole('DOCTOR'); setMobileMenuOpen(false); }}
                className={`py-2 px-2 text-xs font-semibold rounded-lg border text-center ${role === 'DOCTOR' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200'}`}
              >
                Doctor
              </button>
              <button
                onClick={() => { switchDemoRole('ADMIN'); setMobileMenuOpen(false); }}
                className={`py-2 px-2 text-xs font-semibold rounded-lg border text-center ${role === 'ADMIN' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200'}`}
              >
                Admin
              </button>
            </div>
